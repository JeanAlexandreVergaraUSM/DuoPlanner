...
/* ================= Dependencias + estado ================= */
// Reemplaza COMPLETA esta función en js/malla.js
function actualizarDependencias(host){
  const all = host.querySelectorAll('.grid-item');

  // === Detectar carrera actualmente mostrada (MEDVET/ICTEL) ===
  const career = document.getElementById('page-malla')?.dataset.career || '';

  // === Cálculo único: ¿están aprobados TODOS los ramos de 1º a 4º año? (MedVet) ===
  let allApprovedFirst4 = true;
  if (career === 'MEDVET') {
    const first4 = Array.from(all).filter(el => {
  ...
  // Helper: match acento-insensible
  ...
  all.forEach(el=>{
    ...
    // 1) Revisa prerequisitos normales (por key/código/sigla) —igual que antes
    const okPrereqs = prereqs.every(k=>{
      const key = CSS.escape(String(k));
      const byKey = host.querySelector(`.grid-item[data-key="${key}"]`);
      const byCod = host.querySelector(`.grid-item[data-codigo="${key}"]`);
      const bySig = Array.from(host.querySelectorAll('.grid-item'))
        .find(d => d.querySelector('.sigla-label')?.textContent === String(k));
      const pre = byKey || byCod || bySig;
      return pre && pre.classList.contains('aprobado');
    });

    // 2) Regla EXTRA (solo MedVet): “Práctica Profesional” exige 1º–4º año completos
    let extraOk = true;
    if (career === 'MEDVET') {
      const name = norm(el.querySelector('.course-name')?.textContent || '');
      const isPracticaProfesional = name.includes('practica') && name.includes('profesional');
      if (isPracticaProfesional) {
        extraOk = allApprovedFirst4;
      }
    }

    // 3) Aplica bloqueo si no cumple (importante: NO quitamos 'aprobado', seguimos tu criterio)
    const ok = okPrereqs && extraOk;
